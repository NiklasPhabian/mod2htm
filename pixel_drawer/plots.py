...
class Plot:
    ...
    def plot(self, x, y, series_name, linestyle='solid', marker=None):
        line = self.ax.plot(x, y, linestyle=linestyle, marker=marker, markersize=10)[0]
        self.lines.append(line)
        self.legend.append(series_name)
        self.set_xlim(min(x), max(x))

    def plot_pandas(self, series, linestyle='solid', marker=None):
        line = self.ax.plot(series, linestyle=linestyle, marker=marker, markersize=10)[0]
    ...
    def setup_plot(self):
        self.ax.grid('on')
        self.ax.tick_params(labelsize=self.font_size, pad=10)
    ...
    def x_label(self, x_label):
        self.ax.set_xlabel(x_label, fontsize=self.font_size, labelpad=20)
        self.ax.xaxis.set_label_coords(0.9, -0.13)

    def y_label(self, y_label):
        self.ax.set_ylabel(y_label, fontsize=self.font_size, labelpad=20, rotation=0)
        self.ax.yaxis.set_label_coords(-0.15, 0.85)

    def y_label2(self, y_label):
        self.ax2.set_ylabel(y_label, fontsize=self.font_size, labelpad=20, rotation=0)
        self.ax2.yaxis.set_label_coords(1.15, 0.95)

    def make_legend(self):
        self.ax.legend(handles=self.lines, labels=self.legend, fontsize=self.font_size, loc=self.legend_loc)
    ...
    def add_second_y(self):
        self.ax2 = self.ax.twinx()
        self.ax2.tick_params(labelsize=self.font_size, pad=10)
    ...
    def plot_pandas_second_y(self, series, linestyle='-', marker='None'):
        if not self.ax2:
            self.add_second_y()
        line = self.ax2.plot(series, linestyle=linestyle, marker=marker, markersize=20)[0]
        self.lines.append(line)
        self.legend.append(series.name)
...
class Plot3D(Plot):
    ...
    def z_label(self, z_label):
        self.ax.set_zlabel(z_label, fontsize=self.font_size, labelpad=20)
        self.color_bar.set_label(z_label, fontsize=self.font_size)
...
class SpherePlot:
    ...
    def make_sphere(self, finess=20, wire=False, zoom=1, only_north=False, only_front=False):
        ...
        if wire:
            self.ax.plot_wireframe(x, y, z, rstride=1, cstride=1, color='gray', alpha=1, linewidth=self.line_width)
        else:
